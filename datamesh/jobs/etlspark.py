...
def transform_data(df1, steps_per_floor_):
    ...
    df1_3 = df1_2 \
        ...
        .withColumn('Name_Prefix', when(length(trim(col('Name_Prefix'))) == 0, None).otherwise(
        df1_2["Name_Prefix"].cast(StringType()))) \
        .withColumn('First_Name',
                    when(length(trim(col('First_Name'))) == 0, None).otherwise(df1_2["First_Name"].cast(StringType()))) \
        .withColumn('Middle_Initial', when(length(trim(col('Middle_Initial'))) == 0, None).otherwise(
        df1_2["Middle_Initial"].cast(StringType()))) \
        .withColumn('Last_Name',
                    when(length(trim(col('Last_Name'))) == 0, None).otherwise(df1_2["Last_Name"].cast(StringType()))) \
        .withColumn('Gender',
                    when(length(trim(col('Gender'))) == 0, None).otherwise(df1_2["Gender"].cast(StringType()))) \
        .withColumn('Age_in_Yrs',
                    when(length(trim(col('Age_in_Yrs'))) == 0, None).otherwise(df1_2["Age_in_Yrs"].cast(DoubleType()))) \
        .withColumn('Age_in_Company_Yrs', when(length(trim(col('Age_in_Company_Yrs'))) == 0, None).otherwise(
        df1_2["Age_in_Company_Yrs"].cast(DoubleType()))) \
        .withColumn('Salary',
                    when(length(trim(col('Salary'))) == 0, None).otherwise(df1_2["Salary"].cast(DoubleType()))) \
        .withColumn('Place_Name',
                    when(length(trim(col('Place_Name'))) == 0, None).otherwise(df1_2["Place_Name"].cast(StringType()))) \
        .withColumn('County',
                    when(length(trim(col('County'))) == 0, None).otherwise(df1_2["County"].cast(StringType()))) \
